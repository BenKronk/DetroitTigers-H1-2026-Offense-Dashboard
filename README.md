...
- **Benchmarked KPI cards** Runs per Game, Home Runs, OPS, and K%, each showing the Tigers' value alongside the MLB and Central Division figures for context. Directionality is handled correctly: K% is colored inversely (lower is better), unlike the counting and rate stats where higher is better.
- **Slash-line comparison** A clustered bar chart comparing AVG / OBP / SLG across the Tigers, the Central Division, and MLB.
- **Month-over-month heatmaps** Two conditional-formatted matrices showing the Tigers' AVG / OBP / SLG performance *relative to* MLB and to the Central Division, by month, with a diverging red-to-green scale that makes hot and cold stretches immediately visible.
- **Standings context** Monthly Win/Loss record, providing a narrative link between offensive production and results.
- **Month slicer** Filters the KPI cards, slash-line chart, and heatmaps to a selected month or the full season.
- **Player detail table** Full-roster season batting stats (see *Known Limitations* regarding month interactivity).
...
- **Python** data extraction (`MLB-StatsAPI`, `pandas`)
- **Power BI Desktop** Power Query (M) for transformation, DAX for modeling and measures, visualization
...
- **KPI card behavior across multiple months.** The KPI-style visuals display the most recent selected month's value, not a sum across a multi-month selection this is how the Power BI KPI visual works by design. Cards intended to total across a selection use the standard Card visual instead.
...
- `tigers_dashboard_pull_v2.py` main extraction script (season aggregates, monthly tables, players, benchmarks)
- `*.csv` extracted data files consumed by Power BI
- `DetTigersDashboard_H1_2026.pbix` the Power BI report
- `DetTigersDashboard_H1_2026.png` ashboard screenshot
